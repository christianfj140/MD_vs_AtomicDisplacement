#!/usr/bin/env python3
"""Local web UI and API for editing pipeline_config.yaml and running main_md.py."""
...
from md_pipeline_config import command, load_pipeline_config, paths

PROJECT_ROOT = Path(__file__).resolve().parents[1]
CONFIG_PATH = PROJECT_ROOT / "pipeline_config.yaml"
MAIN_SCRIPT = PROJECT_ROOT / "scripts" / "main_md.py"
...
class PipelineRunner:
    ...
    def start(self) -> dict[str, Any]:
        ...
        if not venv_activate.exists():
            raise RuntimeError(
                "No se encontró el script de activación del entorno virtual: "
                f"{venv_activate}"
            )
        ...
        bash_command = (
            ...
            f"&& python {shlex.quote(str(MAIN_SCRIPT))}"
        )

        with self._lock:
            if self._process is not None and self._process.poll() is None:
                raise RuntimeError("El pipeline ya se está ejecutando.")

            self._logs = []
            self._started_at = time.time()
            self._finished_at = None
            self._returncode = None
            self._command = [shell, "-lc", bash_command]
        ...
        thread = threading.Thread(target=self._collect_output, args=(process,), daemon=True)
        thread.start()
        return self.status()
    ...
    def _collect_output(self, process: subprocess.Popen[str]) -> None:
        ...
        with self._lock:
            ...
            if self._process is process:
                self._process = None
            self._logs.append(f"\n[UI] Proceso finalizado con código {returncode}.\n")
...
def set_nested(config: dict[str, Any], path: str, value: Any) -> None:
    ...
    for key in keys[:-1]:
        if not isinstance(target, dict) or key not in target:
            raise RuntimeError(f"No existe la ruta de configuración: {path}")
        target = target[key]
    if not isinstance(target, dict):
        raise RuntimeError(f"No se puede escribir la ruta de configuración: {path}")
    target[keys[-1]] = value
            ...
            if path == "/api/health":
                json_response(
                    self,
                    {
                        ...
                        "venv_activate": str(paths(load_pipeline_config())["venv_activate"]),
                    },
                )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Serve the MD pipeline UI.")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8765)
    args = parser.parse_args()

    server = ThreadingHTTPServer((args.host, args.port), PipelineUIHandler)
    print(f"Pipeline UI listening on http://{args.host}:{args.port}")
